...
// 定时中断，用于扫描点阵屏显示，按行扫描
volatile uint16_t DispRow = 0;
void TIMER_LED_MATRIX_INST_IRQHandler(void) {
    switch ( DL_Timer_getPendingInterrupt(TIMER_LED_MATRIX_INST) ) {
        case DL_TIMER_IIDX_LOAD:
            HC595OutputBits(0, 32); // 在开始新一轮扫描之前，先清空先前的显示
            HC595ShiftBits(__disp_pic[DispRow], 32); // 将新的一行数据写入HC595
            YMQWritePort(DispRow); // 使能新一行的行显
            HC595LatchBits(); // 将HC595的数据锁存到输出端口
            DispRow = (DispRow==31) ? 0 : (DispRow+1); // 行扫描计数器，从0~31循环
            break;
        default: break;
    }
}
...
void LEDMatrix_Init(void) {
    LEDMatrix_ClearAll(); // 清空显存
    NVIC_EnableIRQ(TIMER_LED_MATRIX_INST_INT_IRQN); // 打开定时器中断
    DL_Timer_startCounter(TIMER_LED_MATRIX_INST); // 启动定时器
}
...
/**
 * @brief 绘制一个四边形。
 * @param x0 四边形左上角位置，取值范围[0,31]
 * @param y0 四边形左上角位置，取值范围[0,31]
 * @param x1 四边形右上角位置，取值范围[0,31]
 * @param y1 四边形右上角位置，取值范围[0,31]
 * @param x2 四边形右下角位置，取值范围[0,31]
 * @param y2 四边形右下角位置，取值范围[0,31]
 * @param x3 四边形左下角位置，取值范围[0,31]
 * @param y3 四边形左下角位置，取值范围[0,31]
 */
void LEDMatrix_DrawQuad(int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3) {
    LEDMatrix_DrawLine(x0, y0, x1, y1);
    LEDMatrix_DrawLine(x1, y1, x2, y2);
    LEDMatrix_DrawLine(x2, y2, x3, y3);
    LEDMatrix_DrawLine(x3, y3, x0, y0);
}
