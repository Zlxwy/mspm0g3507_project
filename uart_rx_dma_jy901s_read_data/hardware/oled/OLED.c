...
static int32_t S32ABS(int32_t num){return num>=0?num:-num;}
/*显示有符号小数，一个正负号、size1位整数位、一个小数点、size2位小数位*/
void OLED_ShowFloatNum(uint16_t line, uint16_t column, float num, uint16_t size1, uint16_t size2)
{
  OLED_ShowSignedNum(line, column, (int)num, size1);
  OLED_ShowChar(line, column+size1+1, '.');
  OLED_ShowNum(line, column+size1+2, S32ABS((int32_t)(num*OLED_Pow(10,size2)))%OLED_Pow(10,size2), size2);
}