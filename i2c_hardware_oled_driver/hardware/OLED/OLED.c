/***************************************************************************************
  ...
  * 
  * Program Name:               0.96寸OLED显示屏驱动程序（4针脚I2C接口）
  * Program Creation Date:      2023.10.24
  * Current Program Version:    V2.0
  * Current Version Date:       2024.10.20
  * 
  * Official Website:           https://jiangxiekeji.com
  * Official Taobao Store:      https://jiangxiekeji.taobao.com
  * Program Introduction:       https://jiangxiekeji.com/tutorial/oled.html
  * 
 ...
 * @param abyte 要写入的字节
 */
static void i2c_writeOneByte(uint8_t EquiAddr, uint8_t RegAddr, uint8_t abyte) {
...
/**
 * @brief 指定地址读
 * @param EquiAddr 从机的原7位地址
 * @param RegAddr 从机寄存器的地址
 * @retval 读取的一个字节
 */
static uint8_t i2c_readOneByte(uint8_t EquiAddr, uint8_t RegAddr) {
    ...
    while (DL_I2C_getControllerStatus(I2C_OLED_INST) & DL_I2C_CONTROLLER_STATUS_BUSY_BUS);
    while (!(DL_I2C_getControllerStatus(I2C_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
    DL_I2C_startControllerTransfer(I2C_OLED_INST, EquiAddr, DL_I2C_CONTROLLER_DIRECTION_RX, 1);
    while (DL_I2C_isControllerRXFIFOEmpty(I2C_OLED_INST));
    return DL_I2C_receiveControllerData(I2C_OLED_INST);
}
...
#define OLED_ADDR  0x3C // 未左移的7位原地址 0011 1100

void OLED_WriteCommand(uint8_t command) { i2c_writeOneByte(OLED_ADDR, 0x00, command); }
void OLED_WriteData(uint8_t *Data, uint8_t Count) { i2c_writeBytes(OLED_ADDR, 0x40, Data, Count); }
